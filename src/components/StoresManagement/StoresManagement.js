...
export default class StoreManagement extends Component {
  ...
  theadShow() {
    return this.state.thead.map((item) => <th className={"py-2 py-xl-3 "+((item.id===2 || item.id===3) && "pe-xl-5")} key={item.id}>{item.name}</th>);
  }

  tableDataShow() {
    return this.state.tableData.map((item) => (
      <tr key={item.id}>
        <td className="py-xl-3">{item.number}</td>
        <td className="py-xl-3 pe-xl-5">{item.name}</td>
        <td className="py-xl-3 pe-xl-5">{item.url}</td>
        <td className="py-xl-3"> 
          <div className="d-grid gap-2 d-xl-flex">
            <button type="button" className={"btn py-2 "+style.button_edit}>
                <div className="me-3">
            ...
            <button type="button" className={"btn py-2 "+style.button_delete}>
              <div className="me-3">
